#!/usr/bin/env tsx
/**
 * Romanian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://legislatie.just.ro';
const PORTAL_NAME = 'Legislatie (Romanian Ministry of Justice)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/romanian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Romanian Law MCP — Data Freshness Check');

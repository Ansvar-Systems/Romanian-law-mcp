#!/usr/bin/env tsx
/**
 * Drift detection for Romanian Law MCP.
 *
 * Checks if upstream legislatie.just.ro content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Romanian Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Romanian-Law-MCP/1.0 drift-detect' },
      });

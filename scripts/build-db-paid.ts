#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Romanian Law MCP server.
 *
...
const DEFAULT_DB_PATH = path.resolve(__dirname, '../../../Romanian-law-mcp/data/database.db');
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'rejust',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  proposer TEXT,
  chamber TEXT,
  source TEXT NOT NULL DEFAULT 'cdep_ro',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Romanian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'RO');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'rejust.ro');
    upsertMeta.run('source_prep_works', 'cdep.ro,senat.ro');
    upsertMeta.run('source_agency', 'dataprotection.ro');
  });
  updateMeta();

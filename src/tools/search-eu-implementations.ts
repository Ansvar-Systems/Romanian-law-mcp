/**
 * search_eu_implementations — Search EU directives/regulations referenced by Romanian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_romanian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as romanian_statute_count
  ...
  if (input.has_romanian_implementation) {
    sql += ' HAVING romanian_statute_count > 0';
  }

/**
 * get_romanian_implementations — Find Romanian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetRomanianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface RomanianImplementationResult {
  document_id: string;
...
export async function getRomanianImplementations(
  db: InstanceType<typeof Database>,
  input: GetRomanianImplementationsInput,
): Promise<ToolResponse<RomanianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as RomanianImplementationResult[];
